...
def cost_func(theta, X, y,lam = 0.5):
    m = len(X)
    pred = X.dot(theta)
    cost = (1/m) *((np.sum(np.square(pred - y))) + lam*(np.sum(np.square(theta))))
    return cost**(1/2)

def gradientDescent(X_train, X_test, y_train, y_test, learning_rate=0.1, iteration=1000,lam = 0.5):
    m = len(y_train)
...
theta, rmse_train, rmse_test = gradientDescent(X_train, X_test, y_train, y_test, 0.01, 20000)

y_pred = theta[0] + theta[1] * X_train
plt.plot(X_train, y_pred, color='r', label='Gradient Descent')
X_train = X_train[:,1]
